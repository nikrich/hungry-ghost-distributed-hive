...
vi.mock('../hooks/useRunUpdates', () => ({
  useRunUpdates: () => ({ status: mockStatus, disconnect: vi.fn() }),
}));
...
const testStories: Story[] = [
  ...
  { id: 'STR-002', title: 'Add token refresh', points: 5, status: 'in_progress', assignee: 'senior-1', dependencies: ['STR-001'] },
...
describe('RunView', () => {
  ...
  describe('stories panel', () => {
    ...
    it('shows dependencies for stories with deps', () => {
      seedStore();
      renderRunView();
      expect(screen.getByText('Depends on: STR-001')).toBeInTheDocument();
    });
